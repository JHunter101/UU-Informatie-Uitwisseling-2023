...
function buildCart() {
    ...
    cart.forEach((item) => {
        ...
        const extractedNumber = parseFloat(clothesData[productId]['costs'].slice(clothesData[productId]['costs'].indexOf('\u00a3') + 1));
        ...
        cartItemPriceAll.textContent = `£${(quantity * extractedNumber).toFixed(2)}`;
...
function buildCartTotal(total) {
    const cartTotal = document.getElementById('cart-total-text');
